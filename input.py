...
class Input(object):
    ...
    def check(self):
        ...
        if pm.run.time_dependence == True:
            if pm.run.MBPT == True:
                if pm.mbpt.screening == 'dynamic' or pm.mbpt.screening == 'static':
                    raise AttributeError('Error: Cannot perform TD-MBPT with dynamic/static screening, change pm.mbpt.screening to \'inertial\'')
            if pm.run.HYB == True:
